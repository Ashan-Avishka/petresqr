...
const inputClass = (hasError?: boolean) =>
  `w-full bg-gray-800 text-white rounded-lg px-4 py-2 border focus:outline-none focus:ring-1 transition-colors ${
    hasError
      ? 'border-red-500/70 bg-red-500/5 focus:ring-red-500'
      : 'border-gray-700 focus:ring-primary'
  }`;

const textareaClass = (hasError?: boolean) =>
  `w-full bg-gray-800 text-white rounded-lg px-4 py-2 border focus:outline-none focus:ring-1 min-h-[60px] transition-colors ${
    hasError
      ? 'border-red-500/70 bg-red-500/5 focus:ring-red-500'
      : 'border-gray-700 focus:ring-primary'
  }`;
...
const CreatePetModal: React.FC<CreatePetModalProps> = ({ isOpen, onClose, onSubmit, availableTags }) => {
  ...
  const validateStep = (step: number): boolean => {
    let errors: PetFieldErrors = {};

    if (step === 1) errors = validatePetStep1(formData);
    if (step === 2) errors = validatePetStep2(selectedTag);
    if (step === 3) errors = validatePetStep3(formData.medical);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      submitData.append('gallery', formData.story.enabled ? 'true' : 'false');
  ...
  const handleClose = () => {
    setCurrentStep(1);
    setFormData({
      name: '', type: 'dog', breed: '', age: '', weight: '', gender: 'male', color: '',
      dateOfBirth: '',
      bio: { description: '', microchipId: '' },
      medical: { allergies: '', medications: '', conditions: '', vetName: '', vetPhone: '' },
      other: { favoriteFood: '', behavior: '', specialNeeds: '' },
        ...
        return (
          <div className="space-y-6">
            <div className="flex">
              {/* Photo upload */}
              <div className="w-1/4">
                <label className="block text-sm font-medium text-gray-300 mb-2">Pet Photo</label>
                  ...
                  <div className="flex gap-3">
                    {(['dog', 'cat', 'other'] as const).map((type) => (
                      <button
                        ...
                        className={`flex-1 py-3 rounded-lg flex items-center justify-center gap-2 transition-all capitalize ${
                          formData.type === type
                            ? 'bg-gradient-to-br from-primary via-black to-black text-white shadow-sm shadow-primary'
                            : 'bg-gray-800 text-gray-400 hover:bg-gray-700'
                        }`}
                      >
                        {type === 'dog' ? <Dog className="w-5 h-5" /> : type === 'cat' ? <Cat className="w-5 h-5" /> : <Rabbit className="w-5 h-5" />}
                ...
                {availableTags.map((tag) => (
                  <button
                    ...
                    onClick={() => {
                      ...
                      if (fieldErrors.tag) setFieldErrors((prev) => ({ ...prev, tag: undefined }));
                    }}
                    className={`p-4 rounded-lg border-2 transition-all text-left ${
                      selectedTag === tag._id
                        ? 'border-primary bg-primary/10'
                        : fieldErrors.tag
                        ? 'border-red-500/50 bg-gray-800 hover:border-red-500/70'
          ...
          <div className="space-y-6">
            <div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Veterinarian Name</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Vet Phone</label>
                  <input
                    type="tel"
                    value={formData.medical.vetPhone}
                    onChange={(e) => updateNestedField('medical', 'vetPhone', e.target.value)}
                    className={inputClass(!!fieldErrors.vetPhone)}
                    placeholder="Vet phone number"
                  />
                  <FieldError message={fieldErrors.vetPhone} />
                </div>
              </div>
            </div>
          </div>
      ...
      case 4:
        return (
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Behavior Notes</label>
              <textarea
            ...
            <div>
              <label className="block text-sm font-medium text-gray-300 mb-2">Special Needs</label>
              <textarea
          ...
          <div className="space-y-4">
            ...
            {formData.story.enabled && (
              <div className="space-y-4 pt-4">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-medium text-gray-300 mb-2">Story Status</label>
                    <select
            ...
            <div className="bg-gradient-to-tl from-primary via-black to-black rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden border border-primary/20">
                  ...
                  {steps.map((step, index) => (
                    <React.Fragment key={step.number}>
                      ...
                      {index < steps.length - 1 && (
                        <div className={`h-0.5 flex-1 mx-2 ${currentStep > step.number ? 'bg-primary' : 'bg-gray-800'}`} />
                      )}
                    </React.Fragment>
                  ))}
                </div>
              ...
              <div className="flex items-center justify-between gap-3 p-6 border-t border-gray-800">
                ...
                {currentStep < steps.length ? (
                  ...
                  >
                    {isSubmitting ? 'Creating...' : 'Create Pet'} <Check className="w-4 h-4" />
                  </button>
                )}
              </div>
            </div>
